...
# Download tarball and generate SHA256
TARBALL_URL="https://github.com/${GITHUB_OWNER}/${REPO_NAME}/archive/refs/tags/${NEW_TAG}.tar.gz"
echo -e "${green}Downloading tarball...${nc}"
curl -L -o ${NEW_TAG}.tar.gz "$TARBALL_URL"

SHA256=$(shasum -a 256 ${NEW_TAG}.tar.gz | awk '{print $1}')
echo -e "${green}SHA256: $SHA256${nc}"

# Update formula
echo -e "${green}Updating formula at $TAP_FORMULA_PATH...${nc}"
sed -i.bak -E \
  -e "s|url \\\"https://github.com/.*/.*/archive/refs/tags/v[^\"]+\\.tar\\.gz\\\"|url \\\"$TARBALL_URL\\\"|" \
